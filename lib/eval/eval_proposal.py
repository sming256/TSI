...
class ANETproposal(object):
    ...
    def evaluate(self):
        ...
        if self.verbose:
            ...
            print(
                "\tArea Under the AR vs AN curve: {}%".format(100.0 * float(area_under_curve) / proposals_per_video[-1])
            )

        self.recall = recall
        self.avg_recall = avg_recall
        self.proposals_per_video = proposals_per_video
        self.auc = 100.0 * float(area_under_curve) / proposals_per_video[-1]
        return self.recall, self.auc
