...
class VideoDataSet(torch.utils.data.Dataset):
    def __init__(self, mode="train", subset="train", cfg=None, logger=None):
        ...
        self.mode = mode
        self.subset = subset
        self.datasetlib_path = "./lib/dataset/hacs/data/"
    ...
    def _getDatasetDict(self):
        anno_database = json.load(open(self.datasetlib_path + "HACS_segments_v1.1.1.json"))
        ...
        self.video_dict = {}
        for video_name in list(anno_database.keys()):
            ...
            for gt in video_info["annotations"]:
                tmp_start = max(min(1, float(gt["segment"][0]) / float(video_info["duration"])), 0)
                tmp_end = max(min(1, float(gt["segment"][1]) / float(video_info["duration"])), 0)
                if tmp_end - tmp_start > 0.01:
                    gt_cnt += 1
            if gt_cnt == 0 and video_info["subset"] == "training":
                continue

            if self.subset in video_info["subset"]:
    ...
    def _get_video_label(self, index):
        ...
        for gt in video_labels:
            tmp_start = max(min(1, float(gt["segment"][0]) / video_second), 0)
            tmp_end = max(min(1, float(gt["segment"][1]) / video_second), 0)
    ...
    def _get_base_feat(self, index):
        ...
        if self.online_resize:
            video_data = pickle.load(open(os.path.join(self.feature_path, "{}.pkl".format(video_name)), "rb"))
            slow_feat = video_data["slow_feature"].squeeze(0)
            fast_feat = video_data["fast_feature"].squeeze(0)
            num_snippets = min(slow_feat.shape[1], fast_feat.shape[1])
            video_data = torch.cat((slow_feat[:, :num_snippets], fast_feat[:, :num_snippets]), dim=0)
            video_data = video_data.permute(1, 0).numpy()
